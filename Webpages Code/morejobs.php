...
<div id="postpop-up" class="modal1">
  ...
  <?php
    ...
    // SQL Query
    $query = "SELECT ID, Jobname, Jobdesc, Jobfield, applicant FROM jobposting WHERE ID='$jobid'";
    ...
    if ($result->num_rows > 0) {
        ...
        while($row = $result->fetch_assoc()) {
            echo "<span> <div class='centeralisefields' 
              <br> <h4>Job: ".$row['Jobname']."</h4>
              <br> <h6>Description: ".$row['Jobdesc']."<h6>
              <br> <h4>Field: ".$row['Jobfield']."</h4>
              <br> <h6>Applicant: ".$row['applicant']."</h6>
              <br> </div> </span>";
        }
    } else {
        // If there is an error getting the results
        echo "An error was encountered when fetching your data, please try again";
    }
